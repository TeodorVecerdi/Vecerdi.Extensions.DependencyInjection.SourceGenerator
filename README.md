...
namespace MyProject
{
    public class PlayerController : BaseMonoBehaviour
    {
        ...
        // Collection injection - various collection types are supported
        [Inject] public IEnumerable<IInputHandler> InputHandlers { get; set; }
        [Inject] public IInputHandler[] InputHandlerArray { get; set; }
        [Inject] public List<IAudioSource> AudioSources { get; set; }
        [Inject] public IReadOnlyCollection<IEffect> Effects { get; set; }
        
        // Keyed service injection (single services)
...
public partial class GeneratedTypeInjectorResolver
{
    ...
    private sealed class PlayerControllerInjector : ITypeInjector
    {
        ...
        public void Inject(IServiceProvider serviceProvider, object instance)
        {
            ...
            typedInstance.InputHandlers = serviceProvider.GetServices<IInputHandler>();
            typedInstance.InputHandlerArray = serviceProvider.GetServices<IInputHandler>().ToArray();
            typedInstance.AudioSources = serviceProvider.GetServices<IAudioSource>().ToList();
            typedInstance.Effects = serviceProvider.GetServices<IEffect>().ToArray();
            
            // Single keyed services
            ...
            // Keyed collection injection
            typedInstance.CombatAbilities = serviceProvider.GetKeyedServices<IAbility>("combat");
            typedInstance.UIComponents = serviceProvider.GetKeyedServices<IUIComponent>("ui").ToList();
        }
    }
}
```
