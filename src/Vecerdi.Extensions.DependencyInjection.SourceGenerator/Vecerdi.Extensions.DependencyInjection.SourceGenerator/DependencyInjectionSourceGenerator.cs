    ...
    private static void GenerateCode(SourceProductionContext context, Compilation compilation, ImmutableArray<(ClassDeclarationSyntax ClassSyntax, bool IsContextClass)> contextClassTuples) {
        ...
        foreach (var syntaxTree in compilation.SyntaxTrees) {
            ...
            foreach (var classDecl in root.DescendantNodes().OfType<ClassDeclarationSyntax>()) {
                ...
                if (injectProperties.Any(p => p.Property.SetMethod == null || p.Property.SetMethod.IsInitOnly)) {
                    ...
                    var diagnostic = Diagnostic.Create(new DiagnosticDescriptor("DI001", "Invalid Property", "Type '{0}' has [Inject] properties without setters; skipping generation.", "Warning", DiagnosticSeverity.Warning, true), classDecl.GetLocation(), typeSymbol.Name);
        ...
        foreach (var (classSyntax, _) in contextClassTuples) {
            ...
            if (contextSymbol.TypeParameters.Length > 0) {
                var diagnostic = Diagnostic.Create(new DiagnosticDescriptor("DI002", "Unsupported", "Generic context class '{}' is not supported; skipping.", "Warning", DiagnosticSeverity.Warning, true), classSyntax.GetLocation(), contextSymbol.Name);
                context.ReportDiagnostic(diagnostic);
                continue;
            }
            ...
            if (contextClassTuples.Length > 1) {
                var diagnostic = Diagnostic.Create(new DiagnosticDescriptor("DI003", "Multiple Contexts", "Multiple injection contexts found; ensure they don't conflict.", "Info", DiagnosticSeverity.Info, true), classSyntax.GetLocation());
                context.ReportDiagnostic(diagnostic);
            }
            ...
            if (eligibleTypes.Count == 0) {
                ...
                // Optional warning
                var diagnostic = Diagnostic.Create(new DiagnosticDescriptor("DI004", "No Types", "No eligible types found for context '{0}'.", "Info", DiagnosticSeverity.Info, true), classSyntax.GetLocation(), contextClassName);
                context.ReportDiagnostic(diagnostic);
            } else {
                codeBuilder.AppendLine("""
        ...
        foreach (var prop in typeSymbol.GetMembers().OfType<IPropertySymbol>()) {
            ...
            if (setter == null ||
                ...
                var diagnostic = Diagnostic.Create(
                    new DiagnosticDescriptor("DI005", "Inaccessible Property", $"[Inject] property '{prop.Name}' in type '{typeSymbol.Name}' has inaccessible setter (must be public, internal, or protected internal); skipping property.", "Warning", DiagnosticSeverity.Warning, true),
                    location);
                context.ReportDiagnostic(diagnostic);
                continue;
            }
